    ...
    <div className="rounded-md border bg-card p-5 shadow-[var(--shadow-whisper)]">
      <div className="grid gap-4 md:grid-cols-[minmax(0,1.25fr)_minmax(0,0.9fr)_minmax(0,1fr)_minmax(0,1.1fr)] md:items-center">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="num truncate text-base font-medium text-ink">{server.name}</span>
            <span
        ...
        <div>
          ...
          <p className="num mt-1.5 text-xs text-faint">uptime {formatUptime(server.uptimePct)}</p>
        </div>

        {/* Latencia + sparkline */}
        <div className="flex items-center gap-3">
          <div className="whitespace-nowrap">
            <span className={cn('num font-display text-lg font-semibold', latencyTone(server.latencyMs))}>
              {server.latencyMs}
            </span>
