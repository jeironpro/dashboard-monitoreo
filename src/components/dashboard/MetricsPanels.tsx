            ...
            <div key={db.id} className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="num truncate text-sm font-medium text-ink">{db.name}</p>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                <span>
                  <span className="num text-ink">{db.queryLatencyMs} ms</span> query
                </span>
                <span>
                  <span className="num text-ink">{db.slowQueries}</span> lentas
                </span>
                <span>
                  <span className="num text-ink">{db.replicationLagSec} s</span> lag
                </span>
              </div>
            </div>
          )
        ...
        {queues.map((queue) => {
          ...
          return (
            <div key={queue.id} className="space-y-1.5">
              <div className="flex items-center justify-between gap-3">
                <p className="num truncate text-sm font-medium text-ink">{queue.name}</p>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                ...
                <span>
                  <span className={cn('num', queue.failed > 0 && 'text-danger')}>{queue.failed}</span>{' '}
                  fallidos
                </span>
              </div>
  ...
  return (
    <Card className="md:col-span-2">
      <CardHeader>
        ...
        <CardDescription>Latencia, tráfico, tasa de errores y disponibilidad por endpoint</CardDescription>
      </CardHeader>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden text-right lg:table-cell">Disponibilidad</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {apis.map((api) => (
